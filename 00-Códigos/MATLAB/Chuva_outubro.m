...
field5_filtrado = T.field5(idx);

% Remover valores NaN e valores anômalos (ex: fora de uma faixa de 0 a 200 mm para Precipitação)
mask = ~isnan(field5_filtrado) & field5_filtrado >= 0 & field5_filtrado <= 200;
datas_limpas = datas_filtradas(mask);
field5_limpas = field5_filtrado(mask);

% Parâmetros do filtro (tamanho da janela)
windowSize = 5;  % Ajuste conforme necessário

% 1. Filtro de Média Móvel
field5_media_movel = movmean(field5_limpas, windowSize);

% 2. Filtro Mediana
field5_mediana = medfilt1(field5_limpas, windowSize);

% Gerar os gráficos diretamente no layout atual (sem nova janela)

% Gráfico original (Laranja)
subplot(3,1,1);
plot(datas_limpas, field5_limpas, 'Color', [1, 0.5, 0]);  % Laranja
...
subplot(3,1,2);
plot(datas_limpas, field5_media_movel, 'Color', [0, 0.4470, 0.7410]);  % Azul
...
subplot(3,1,3);
plot(datas_limpas, field5_mediana, 'Color', [0.4660, 0.6740, 0.1880]);  % Verde
