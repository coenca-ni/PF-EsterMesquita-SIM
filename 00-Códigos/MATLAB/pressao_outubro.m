...
field3_filtrado = T.field3(idx);

% Remover valores NaN e valores anômalos (ex: fora de uma faixa de 800 a 1200 hPa para Pressão)
mask = ~isnan(field3_filtrado) & field3_filtrado >= 800 & field3_filtrado <= 1200;
datas_limpas = datas_filtradas(mask);
field3_limpas = field3_filtrado(mask);

% Parâmetros do filtro (tamanho da janela)
windowSize = 5;  % Ajuste conforme necessário

% 1. Filtro de Média Móvel
field3_media_movel = movmean(field3_limpas, windowSize);

% 2. Filtro Mediana
field3_mediana = medfilt1(field3_limpas, windowSize);

% Gerar os gráficos diretamente no layout atual (sem nova janela)

% Gráfico original (Laranja)
subplot(3,1,1);
plot(datas_limpas, field3_limpas, 'Color', [1, 0.5, 0]);  % Laranja
...
subplot(3,1,2);
plot(datas_limpas, field3_media_movel, 'Color', [0, 0.4470, 0.7410]);  % Azul
...
subplot(3,1,3);
plot(datas_limpas, field3_mediana, 'Color', [0.4660, 0.6740, 0.1880]);  % Verde
